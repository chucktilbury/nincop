/**
 * @file ./out/parser/directive_definition.c
 *
 * This is a generated file. If you edit this file, don't run the generator
 * in this directory. Run it in a different one and then merge the results
 * using a tool like diff.
 *
 * @date Wed May 14 14:31:37 2025
 * @author Chuck Tilbury
 *
 */

#include <stdio.h>
#include <stdlib.h>

#include "common.h"
#include "parser_protos.h"

/*
 * directive_definition ( TOK_ADD_SEARCH TOK_OPAREN TOK_STRING_LITERAL TOK_CPAREN ) 
 */
ast_directive_definition_t* parse_directive_definition(parser_state_t* pstate) {

ENTER;
ASSERT(pstate != NULL, "null pstate is not allowed");
ast_directive_definition_t* retv = NULL;
int state = 0;
bool finished = false;
void* post = mark_token_queue();

// token_t* TERMINAL_OPER = NULL;
// token_t* STRING_LITERAL = NULL;
// token_t* TERMINAL_OPER = NULL;


while(!finished) {
    switch(state) {

// begin grouping_function
    // terminal rule element: TOK_ADD_SEARCH
    // terminal rule element: TOK_OPAREN
    // terminal rule element: TOK_STRING_LITERAL
    // terminal rule element: TOK_CPAREN
// end grouping_function


        case STATE_MATCH:
            TRACE_STATE;
            consume_token_queue();
            retv = (ast_directive_definition_t*)create_ast_node(AST_DIRECTIVE_DEFINITION);
// retv->TERMINAL_OPER = TERMINAL_OPER;
// retv->STRING_LITERAL = STRING_LITERAL;
// retv->TERMINAL_OPER = TERMINAL_OPER;

            break;
        case STATE_NO_MATCH:
            TRACE_STATE;
            restore_token_queue(post);
            finished = true;
            break;
        case STATE_ERROR:
            TRACE_STATE;
            restore_token_queue(post);
            recover_parser_error(pstate);
            finished = true;
            break;
        default:
            FATAL("unknown state: %d", state);
    }
}

RETURN(retv);
}

